...
| runtime-contract upgrade | 68.1 s | 旧stamp schemaから両contextを再生成 |
| runtime-contract rerun | 6.1-6.3 s | 11資産とfingerprint一致済み両contextを再利用 |
...
- 約2.5 GiBの空き領域
- テスト済み: Python 3.12.3、ONNX 1.22.0、ONNX Runtime 1.27.0、onnxruntime-qnn 2.3.0、tokenizers 0.23.1
...
| QAIRT version | 検証packageが公開しないため`null` |
| Qualcomm QNN runtime version | 検証packageが公開しないため`null` |

installerはonnxruntime-qnn package versionからQAIRTまたはQualcomm QNN runtime versionを推測しません。Python/package version、QNN libraryの絶対pathとhash、target、provider option、session config、最終`ADSP_LIBRARY_PATH`、chunk、total lengthを`install-result.json`、各`source-stamp.json`、server evidenceへ記録します。既存の`ADSP_LIBRARY_PATH` entryは保持し、重複を除きます。runtime identityが変化した場合やstampが壊れている場合は、EPContextを再利用せず再生成します。

既定model URLはHugging Face revision `773ff42cc383cb61ecf32eb13d1f828634fbd0e1`へ固定しています。`--model-base-url`、`--model-repository`、`--model-revision`は明示的なoverrideとして残しています。
...
`install.sh`は既定で公開Hugging Faceの固定revisionから資産を取得し、依存確認、11資産のSHA-256検証、デバイス上EPContext生成を行い、生成・再ロード・実行がfiniteかつQNN-onlyであることを必須にします。決定論的semantic canaryでは通常応答に`Tokyo`を含むこと、JSON応答が`{"answer":"Tokyo"}`であること、first token IDが`40550`であることを要求します。mirrorには`--model-base-url`、offline installには`--asset-dir`を指定できます。失敗時は`dependencies`、`assets`、`epcontext`、`smoke`の段階名を表示します。
