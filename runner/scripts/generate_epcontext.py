...
def make_session(model_path: Path, log_dir: Path, label: str, session_configs=None, provider_options_extra=None):
    ...
    for key, value in (session_configs or {}).items():
...
def main() -> int:
    ...
    try:
        ...
        result["summary"] = {
            "generate_ok": next((c.get("ok") for c in result["cases"] if c.get("label") == "generate_external_epcontext"), False),
            "load_ok": next((c.get("ok") for c in result["cases"] if c.get("label") == "load_external_epcontext"), False),
            "load_qnn_only": next((c.get("qnn_only") for c in result["cases"] if c.get("label") == "load_external_epcontext"), False),
            "load_session_create_s": next((c.get("session_create_s") for c in result["cases"] if c.get("label") == "load_external_epcontext"), None),
            "load_run_s": next((c.get("run_s") for c in result["cases"] if c.get("label") == "load_external_epcontext"), None),
        }
    ...
    print(json.dumps({"ok": "error" not in result, "log_dir": str(log_dir), "summary": str(log_dir / "summary.md")}, ensure_ascii=False), flush=True)
